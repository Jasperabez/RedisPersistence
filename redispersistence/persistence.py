...
def checkIfConvStatePromise(value):
    return isinstance(value, tuple) and isinstance(
        value[1], Promise) and isinstance(value[0], Tuple) and isinstance(
            value[0][0], int) and isinstance(value[0][1], Promise)
...
class RedisPersistence(BasePersistence):
    ...
    def load_redis(self) -> None:
        try:
            ...
            raise TypeError(
                f"Something went wrong unpickling from Redis") from exc
    ...
    def dump_redis(self) -> None:
        ...
        try:
            ...
            # workaround to pickle conversations when run_async is enabled
            for conversation_handler in data['conversations']:
                for key, value in data['conversations'][conversation_handler].items():
                    if checkIfConvStatePromise(value):
                        data['conversations'][conversation_handler][key] = value[0][0]
            data_bytes = pickle.dumps(data)
    ...
    def update_conversation(self, name: str, key: Tuple[int, ...], new_state: Optional[object]) -> None:
        ...
        if not self.conversations:
            self.conversations = dict()
            # restructure object (promise) that contains thread_lock (unpickable)
        if checkIfConvStatePromise(new_state):
